...
def add_info():
    ...
    B = User(user_id='小明',
             ...
             token='***',
             terminal='Chrome')
    Book1 = Book(book_id=0, title='数据结构')
    Book2 = Book(book_id=1, title='PRML')
    session.add_all([A, B, Book1, Book2])
    session.commit()
    StoreA = Store(store_id='王掌柜的书店', book_id=0, stock_level=10,
                   price=1000)  # 价格单位是分
    StoreB = Store(store_id='王掌柜的进口书店', book_id=1, stock_level=10, price=10000)
    session.add_all([StoreA, StoreB])
    session.commit()
    A_Store1 = User_store(user_id='王掌柜', store_id='王掌柜的书店')
    A_Store2 = User_store(user_id='王掌柜', store_id='王掌柜的进口书店')
