...
def add_info():
    ...
    session.commit()
    A_Store1 = User_store(user_id='A', store_id='StoreA')
    A_Store2 = User_store(user_id='A', store_id='StoreB')
    OrderA = New_order_paid(
        order_id='order1',
        buyer_id='B',
        seller_id='A',
        price=20,
    ...
    Order_detailA = New_order_detail(order_id='order1',
                                     ...
                                     count=2,
                                     price=20)
    session.add_all([A_Store1, A_Store2, OrderA, Order_detailA])
    session.commit()
    # 关闭session
    session.close()
